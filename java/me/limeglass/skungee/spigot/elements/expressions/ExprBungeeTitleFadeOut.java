...
@Name("Bungeecord title fade out")
@Description("Returns the fade out time(s) of defined skungee title(s).")
@Properties({"skungeetitles", "fade out[ ][time][s]", "{1}[(all [[of] the]|the)]"})
@PropertiesAddition("[(skungee|bungee[[ ]cord])] title[s]")
@AllChangers
@Disabled
public class ExprBungeeTitleFadeOut extends SkungeePropertyExpression<SkungeeTitle, Timespan> {
	...
	private Map<SkungeeTitle, Timespan> getTimespans(SkungeeTitle[] titles) {
		...
		for (SkungeeTitle title : titles) {
			times.put(title, new Timespan(title.getFadeOut()));
		}
		return times;
	}
	...
	public void change(Event event, Object[] delta, ChangeMode mode) {
		...
		switch (mode) {
			case ADD:
				for (Entry<SkungeeTitle, Timespan> entry : times.entrySet()) {
					if (entry.getValue().getTicks_i() == timespan.getTicks_i()) {
						entry.getKey().setFadeOut((int)(entry.getValue().getTicks_i() + timespan.getTicks_i()));
					}
				}
				break;
			case REMOVE:
			...
			case SET:
				for (SkungeeTitle title : titles) {
					title.setFadeOut((int)timespan.getTicks_i());
				}
				break;
		}
	}
