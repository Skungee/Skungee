...
public class YamlHandler extends SkungeeProxyHandler {
	...
	@Override
	public Object[] handlePacket(ServerPacket input, InetAddress address) {
		YamlPacket packet = (YamlPacket) input;
		if (packet.isUnset())
			return null; //node, path or state could be null.
		...
		try {
			...
			if (configuration.get(node) == null && mode == null) {
				platform.consoleMessage("The file \"" + path + "\" does not contain the node \"" + node + "\"");
				return null;
			}
			...
			platform.exception(error, "There was an error attempting to grab the yaml node \"" + node + "\" from " + path);
			return null;
		}
		if (mode == null) {
			...
			if (file != null)
				try {
					...
					platform.consoleMessage("There was an error attempting to save configuration at " + file.getPath());
				}
		}
		return null;
	}
