...
public class ServerInstancesSockets {
	...
	private static Socket getBootstap() {
		for (int i = 0; i < SkungeeBungee.getConfig().getInt("Recievers.allowedTrys", 5); i++) {
			try {
	...
	public static Object sendPacket(ServerInstancesPacket packet) {
		...
		try {
			...
			if (configuration.getBoolean("security.password.enabled", false)) {
				byte[] password = SkungeeBungee.getEncrypter().serialize(configuration.getString("security.password.password"));
				if (configuration.getBoolean("security.password.hash", true)) {
					if (configuration.getBoolean("security.password.hashFile", false) && SkungeeBungee.getEncrypter().isFileHashed()) {
						password = SkungeeBungee.getEncrypter().getHashFromFile();
					} else {
						password = SkungeeBungee.getEncrypter().hash();
					}
				}
				if (password != null)
			...
			if (configuration.getBoolean("security.encryption.enabled", false)) {
				byte[] serialized = SkungeeBungee.getEncrypter().serialize(packet);
			...
			if (packet.isReturnable()) {
				if (configuration.getBoolean("security.encryption.enabled", false)) {
					...
					return SkungeeBungee.getEncrypter().deserialize(decoded);
				} else {
	...
	public static Object send(ServerInstancesPacket packet) {
		...
		ProxyServer.getInstance().getScheduler().runAsync(SkungeeBungee.getInstance(), new Runnable() {
			@Override
			public void run() {
				sendPacket(packet);
			}
