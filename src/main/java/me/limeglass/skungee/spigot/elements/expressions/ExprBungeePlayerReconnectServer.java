...
public class ExprBungeePlayerReconnectServer extends SkungeePropertyExpression<Object, String> {
	...
	protected String[] get(Event event, Object[] skungeePlayers) {
		...
		Set<String> servers = (Set<String>) sockets.send(new ServerPacket(true, ServerPacketType.PLAYERRECONNECTSERVER, Utils.toSkungeePlayers(skungeePlayers)));
	...
	public void change(Event event, Object[] delta, ChangeMode mode) {
		...
		if (isNull(event) || delta == null || changer == null)
			return;
		sockets.send(new ServerPacket(false, ServerPacketType.PLAYERRECONNECTSERVER, (String) delta[0], null, changer, Utils.toSkungeePlayers(getExpr().getAll(event))));
	}
